...
import {
  DOYOON_PAID_INTRO_STEPS,
  DOYOON_PAID_INTRO_CHAR_DELAY,
  DOYOON_PAID_INTRO_CROSSFADE_ENTER_STEPS,
} from "./paidIntroSteps";

// 연우 paid-intro와 동일 구조 (캐릭터별 STEPS만 다름).
...
export function usePaidIntroScene() {
  ...
  const step = DOYOON_PAID_INTRO_STEPS[stepIndex];
  ...
  const images = useMemo(() => DOYOON_PAID_INTRO_STEPS.map((s) => s.bg), []);
  ...
  const goToStep = useCallback((next: number) => {
    if (next >= DOYOON_PAID_INTRO_STEPS.length) return;

    if (DOYOON_PAID_INTRO_CROSSFADE_ENTER_STEPS.has(next)) {
      setCrossFading(true);
      setTimeout(() => {
        setStepIndex(next);
        setLineIndex(0);
